...
async def generate_notes(
    file: UploadFile = File(...),
    template: str = Form("Standard Study Notes"),
    context_files: Optional[List[UploadFile]] = File(default=[]),
):
    try:
        is_chunk = file.filename == "chunk.webm"
        audio_bytes = await file.read()
        ...
        # Offload the LLM pipeline to a background thread, passing context_text
        result = await asyncio.to_thread(run_pipeline, text, template, context_text)
        result["transcript"] = text
        return result
...
async def process_raw_text(
    ...
    try:
        ...
        result = await asyncio.to_thread(
            run_pipeline, transcript, template, context_text
        )
        result["transcript"] = transcript
        return result
