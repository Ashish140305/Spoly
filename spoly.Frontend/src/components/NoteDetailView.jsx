...
import { Headphones, AlignLeft, Copy, Workflow, Code, DownloadCloud, Edit3, Eye } from 'lucide-react';
import MermaidDiagram from './MermaidDiagram';

export default function NoteDetailView({ isDarkMode, selectedNote, exportFormat, showToast }) {
  
  // 🌟 FIX: We initialize with a default placeholder if the note has no graph
  ...
  const handleDownloadAudio = () => {
    if (!selectedNote.audioUrl) {
      showToast("No audio file available for this note. (Demo notes don't contain audio)");
      return;
    }
    ...
    const safeTitle = selectedNote.title.replace(/[^a-z0-9]/gi, '_').toLowerCase();
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="space-y-8 pb-32">
      ...
      <div className="grid xl:grid-cols-2 gap-8">
        ...
        {/* SUMMARY SECTION */}
        <div className={`shadow-lg rounded-[2rem] p-8 border ${isDarkMode ? "bg-[#1a1f2e] border-[#2A2F3D]" : "bg-white border-slate-200"}`}>
        ...
        {/* 🌟 FIX: DIAGRAM SECTION IS NOW ALWAYS RENDERED */}
        <div className={`shadow-lg rounded-[2rem] p-8 flex flex-col border ${isDarkMode ? "bg-[#1a1f2e] border-[#2A2F3D]" : "bg-white border-slate-200"}`}>
          ...
          <div className={`flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6 border-b pb-4 ${isDarkMode ? "border-[#2A2F3D]" : "border-slate-100"}`}>
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setIsEditingGraph(!isEditingGraph)}
                className={`flex items-center gap-2 px-4 py-2 rounded-xl font-bold text-xs transition-colors border focus:outline-none focus:ring-2 focus:ring-blue-500 ${isDarkMode ? (isEditingGraph ? "bg-blue-600 border-blue-500 text-white" : "bg-[#131722] hover:bg-[#232a3b] text-blue-400 border-[#2A2F3D]") : (isEditingGraph ? "bg-blue-500 border-blue-400 text-white" : "bg-blue-50 hover:bg-blue-100 text-blue-600 border-blue-200")}`}
              >
                {isEditingGraph ? <Eye size={14} /> : <Edit3 size={14} />} {isEditingGraph ? "Preview Diagram" : "Edit Code"}
              </button>
          ...
          <div className={`flex-1 w-full shadow-inner rounded-2xl flex flex-col overflow-hidden min-h-[400px] border ${isDarkMode ? "bg-[#0b0f19] border-[#2A2F3D]" : "bg-slate-50 border-slate-100"}`}>
            {isEditingGraph ? (
              <textarea 
                value={editedGraph}
