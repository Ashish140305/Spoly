...
import { 
  ...
  Languages, FileUp, RefreshCw, Headphones, Youtube, Play, Pause, Activity, Video, FolderSearch, Maximize2, ArrowDown 
} from "lucide-react";
...
const LiveAudioVisualizer = ({ stream, isRecording, colorClass }) => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < numBars; i++) {
        ...
        if(canvasCtx.roundRect) {
          canvasCtx.roundRect(x, (canvas.height - barHeight) / 2, barWidth, barHeight, 2);
        } else {
  ...
  if (!stream) return <AudioWaveform isRecording={isRecording} color={colorClass} />;
  return <canvas ref={canvasRef} width="120" height="32" className="w-full h-8 opacity-90" />;
};
...
export default function LiveNotes() {
  ...
  const [savedNotes, setSavedNotes] = useState([
    { 
      id: 1, title: "OS Process Scheduling", date: "Mar 12, 2026", folderId: "personal", duration: "45:20", items: 4, tags: ["Education", "OS", "Urgent"], 
      summary: "Detailed breakdown of process scheduling algorithms including Round Robin, FCFS, and SJF.", 
      takeaways: "• SJF provides the minimum average waiting time.\n• Round Robin is essential for time-sharing systems.", 
  ...
  const getDiagramForTemplate = (step) => {
    ...
    if (step === 1) return "graph TD;\n  Client-->API_Gateway;\n  API_Gateway-->Auth_Service;";
    if (step === 2) return "graph TD;\n  Client-->API_Gateway;\n  API_Gateway-->Auth_Service;\n  Auth_Service-->DB[(PostgreSQL)];";
        ...
        if (isImage) { setTimeout(() => finishProcessing(), 3000); } 
          ...
          typingRef.current = setInterval(() => {
            ...
            if (!milestones.current.summary && i > 50) {
              ...
              setEditableMermaid(getDiagramForTemplate(1)); setActionItems([{ id: 1, text: "Configure API Gateway", done: false }]);
            }
  ...
  const triggerRemix = (template) => { 
    ...
    setTimeout(() => { 
      ...
      const cleanOldSummary = meetingNotes.summary.replace(/^✨ Document dynamically restructured to fit the .*? framework\.\n\n/, '');
          ...
          <div className="flex items-center gap-4">
            ...
            {(status === 'recording' || status === 'paused') && activeTab !== 'workspace' && (
              ...
              >
                ...
                <span className={`font-mono text-sm font-bold tracking-tight ${status === "paused" ? "text-amber-500" : isDarkMode ? "text-white" : "text-slate-900"}`}>
                  {formatTime(timer)}
                </span>
            ...
            {selectedNote && ( <button onClick={() => setSelectedNote(null)} className={`flex items-center gap-2 px-4 py-2 rounded-xl font-bold transition-all focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-[#0b0f19] ${isDarkMode ? "bg-[#1a1f2e] hover:bg-[#232a3b] text-slate-200 border border-[#232a3b]" : "bg-slate-100 hover:bg-slate-200 text-slate-700 border border-transparent"}`}><ChevronLeft size={16} /> Back to Library</button> )}
          </div>
        </header>
              ...
              <motion.div key="workspace-tab" variants={pageVariants} initial="initial" animate="animate" exit="exit" className="space-y-8">
                  ...
                  <motion.div key="recording-panel" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className={`shadow-xl rounded-2xl p-8 space-y-8 relative overflow-hidden transition-colors border ${isDarkMode ? "bg-[#131722] border-[#232a3b]" : "bg-white/70 backdrop-blur-xl border-white"}`}>
                    ...
                    <AnimatePresence>
                      {activeAiTemplate && status === "idle" && (
                        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className={`mb-6 p-4 rounded-2xl border flex items-center justify-between shadow-sm ${isDarkMode ? "bg-indigo-900/20 border-indigo-800/30" : "bg-indigo-50 border-indigo-200"}`}>
                          <div className="flex items-center gap-4">
                            <div className={`w-12 h-12 flex items-center justify-center rounded-xl ${isDarkMode ? "bg-indigo-900/50 text-indigo-400" : "bg-indigo-100 text-indigo-600"}`}>
                              {activeAiTemplate.icon}
                            </div>
                            <div>
                              <p className={`text-[10px] font-bold uppercase tracking-widest ${isDarkMode ? "text-indigo-400" : "text-indigo-500"}`}>Active Template</p>
                              <h3 className={`text-lg font-bold ${isDarkMode ? "text-indigo-100" : "text-indigo-900"}`}>{activeAiTemplate.name}</h3>
                            </div>
                          </div>
                          <button onClick={() => setActiveAiTemplate(null)} className={`p-2 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 ${isDarkMode ? "hover:bg-indigo-900/50 text-indigo-400" : "hover:bg-indigo-200 text-indigo-600"}`}>
                            <X size={20} />
                          </button>
                        </motion.div>
                      )}
                    ...
                    {status === "idle" && (
                      <motion.div key="idle-buttons" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2 pb-6">
                          ...
                          <motion.button whileHover={{ scale: 1.02, y: -4 }} whileTap={{ scale: 0.97 }} onClick={handleToggleWidget} className={`group relative flex flex-col text-left transition-all duration-300 hover:shadow-2xl border rounded-2xl overflow-hidden min-h-[260px] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-[#131722] ${isWidgetDeployed ? (isDarkMode ? "bg-red-950/20 border-red-900" : "bg-red-50 border-red-200") : isDarkMode ? "bg-[#1a1f2e] border-[#232a3b] hover:border-indigo-600" : "bg-white border-slate-200 hover:border-indigo-400 shadow-sm"}`}>
                            <div className={`w-full h-32 relative flex items-center justify-center overflow-hidden border-b ${isWidgetDeployed ? (isDarkMode ? "bg-[#0b0f19] border-red-900/50" : "bg-red-50 border-red-200") : isDarkMode ? "bg-[#0b0f19] border-[#232a3b]" : "bg-slate-50 border-slate-100"}`}>
                              <div className={`absolute -top-10 -left-10 w-40 h-40 blur-3xl rounded-full transition-colors duration-700 ${isWidgetDeployed ? (isDarkMode ? "bg-red-600/40" : "bg-red-400/40") : isDarkMode ? "bg-purple-600/40" : "bg-purple-300/40"}`}></div>
                              <div className={`absolute -bottom-10 -right-10 w-40 h-40 blur-3xl rounded-full transition-colors duration-700 ${isWidgetDeployed ? (isDarkMode ? "bg-orange-600/40" : "bg-orange-400/40") : isDarkMode ? "bg-indigo-600/40" : "bg-indigo-300/40"}`}></div>
                              
                              <Sparkles className={`absolute left-6 bottom-2 w-24 h-24 transform -rotate-12 group-hover:rotate-12 group-hover:scale-110 transition-all duration-700 ${isWidgetDeployed ? (isDarkMode ? "text-orange-500/25" : "text-orange-500/20") : isDarkMode ? "text-purple-400/25" : "text-purple-500/20"}`} strokeWidth="2" />
                              <Workflow className={`absolute right-6 top-4 w-24 h-24 transform rotate-12 group-hover:rotate-0 group-hover:scale-110 transition-all duration-700 ${isWidgetDeployed ? (isDarkMode ? "text-red-500/25" : "text-red-500/20") : isDarkMode ? "text-indigo-400/25" : "text-indigo-500/20"}`} strokeWidth="2" />
                          ...
                          <motion.button whileHover={{ scale: 1.02, y: -4 }} whileTap={{ scale: 0.97 }} onClick={handleStartLocalRecording} className={`group relative flex flex-col text-left transition-all duration-300 hover:shadow-2xl border rounded-2xl overflow-hidden min-h-[260px] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-[#131722] ${isDarkMode ? "bg-[#1a1f2e] border-[#232a3b] hover:border-blue-500/50" : "bg-white border-slate-200 hover:border-blue-300 shadow-sm"}`}>
                            <div className={`w-full h-32 relative flex items-center justify-center overflow-hidden border-b ${isDarkMode ? "bg-[#0b0f19] border-[#232a3b]" : "bg-slate-50 border-slate-100"}`}>
                              ...
                              <div className={`absolute -bottom-10 -right-10 w-40 h-40 blur-3xl rounded-full transition-colors duration-700 ${isDarkMode ? "bg-cyan-600/40" : "bg-cyan-400/40"}`}></div>
                              ...
                              <Activity className={`absolute right-6 top-4 w-24 h-24 transform rotate-12 group-hover:rotate-0 group-hover:scale-110 transition-all duration-700 ${isDarkMode ? "text-cyan-400/25" : "text-cyan-500/20"}`} strokeWidth="2" />
                          ...
                          <motion.div whileHover={{ scale: 1.02, y: -4 }} whileTap={{ scale: 0.97 }} onDragOver={(e) => { e.preventDefault(); setIsDragging(true); }} onDragLeave={(e) => { e.preventDefault(); setIsDragging(false); }} onDrop={handleDrop} onClick={() => fileInputRef.current.click()} className={`group relative flex flex-col text-left transition-all duration-300 hover:shadow-2xl border border-dashed cursor-pointer rounded-2xl overflow-hidden min-h-[260px] focus-within:ring-2 focus-within:ring-blue-500 focus-within:ring-offset-2 dark:focus-within:ring-offset-[#131722] ${isDragging ? (isDarkMode ? "border-emerald-400 bg-emerald-900/20" : "border-emerald-400 bg-emerald-50") : isDarkMode ? "bg-[#1a1f2e] border-[#232a3b] hover:border-emerald-500/50" : "bg-white border-slate-300 hover:border-emerald-400 shadow-sm"}`}>
                            ...
                            <div className={`w-full h-32 relative flex items-center justify-center overflow-hidden border-b ${isDragging ? (isDarkMode ? "bg-emerald-900/30 border-emerald-800" : "bg-emerald-100/50 border-emerald-200") : isDarkMode ? "bg-[#0b0f19] border-[#232a3b]" : "bg-slate-50 border-slate-100"}`}>
                              ...
                              <div className={`absolute -bottom-10 -right-10 w-32 h-32 blur-3xl rounded-full transition-colors duration-700 ${isDarkMode ? "bg-teal-600/40" : "bg-teal-400/40"}`}></div>
                              
                              <Database className={`absolute left-6 bottom-2 w-20 h-20 transform -rotate-12 group-hover:rotate-12 group-hover:scale-110 transition-all duration-700 ${isDarkMode ? "text-emerald-400/25" : "text-emerald-500/20"}`} strokeWidth="2" />
                              <FileText className={`absolute right-6 top-4 w-20 h-20 transform rotate-12 group-hover:rotate-0 group-hover:scale-110 transition-all duration-700 ${isDarkMode ? "text-teal-400/25" : "text-teal-500/20"}`} strokeWidth="2" />
                          ...
                          <motion.div whileHover={{ scale: 1.02, y: -4 }} className={`group relative flex flex-col text-left transition-all duration-300 hover:shadow-2xl border rounded-2xl overflow-hidden min-h-[260px] ${isDarkMode ? "bg-[#1a1f2e] border-[#232a3b] hover:border-red-500/50" : "bg-white border-slate-200 hover:border-red-300 shadow-sm"}`}>
                            <div className={`w-full h-32 relative flex items-center justify-center overflow-hidden border-b ${isDarkMode ? "bg-[#0b0f19] border-[#232a3b]" : "bg-slate-50 border-slate-100"}`}>
                              <div className={`absolute -top-10 -left-10 w-32 h-32 blur-3xl rounded-full transition-colors duration-700 ${isDarkMode ? "bg-red-600/40" : "bg-red-400/40"}`}></div>
                              <div className={`absolute -bottom-10 -right-10 w-32 h-32 blur-3xl rounded-full transition-colors duration-700 ${isDarkMode ? "bg-orange-600/40" : "bg-orange-400/40"}`}></div>
                              
                              <PlayCircle className={`absolute left-6 bottom-2 w-20 h-20 transform -rotate-12 group-hover:rotate-12 group-hover:scale-110 transition-all duration-700 ${isDarkMode ? "text-red-400/25" : "text-red-500/20"}`} strokeWidth="2" />
                              <Video className={`absolute right-6 top-4 w-20 h-20 transform rotate-12 group-hover:rotate-0 group-hover:scale-110 transition-all duration-700 ${isDarkMode ? "text-orange-400/25" : "text-orange-500/20"}`} strokeWidth="2" />
                            ...
                            <div className="p-6 flex-1 flex flex-col w-full relative z-20">
                              ...
                              <form onSubmit={processYoutube} className="w-full relative mt-auto group/form">
                                ...
                                <button type="submit" className={`absolute right-1.5 top-1/2 -translate-y-1/2 p-2 rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-red-500 ${youtubeUrl ? "bg-red-500 text-white hover:bg-red-600 shadow-md" : isDarkMode ? "text-slate-600 bg-transparent" : "text-slate-400 bg-transparent"}`}><ArrowRight size={16} /></button>
                              </form>
                            </div>
                          </motion.div>
                        </div>
                        ...
                        <div className={`group rounded-2xl p-6 transition-colors border relative overflow-hidden ${isDarkMode ? "bg-[#1a1f2e] border-[#232a3b]" : "bg-white border-slate-200 shadow-sm"}`}>
                          ...
                          <div className={`absolute bottom-0 left-0 w-64 h-64 blur-[80px] rounded-full transition-colors duration-700 pointer-events-none ${isDarkMode ? "bg-purple-600/20 group-hover:bg-purple-500/30" : "bg-purple-300/30 group-hover:bg-purple-300/40"}`}></div>
                          ...
                          <div className="relative z-10">
                            <div className={`flex items-center gap-2 mb-4 font-bold ${isDarkMode ? "text-white" : "text-slate-800"}`}><FileUp size={20} className={isDarkMode ? "text-indigo-400" : "text-indigo-500"} /> Add Pre-Context Documents</div>
                            ...
                            <button onClick={() => contextInputRef.current.click()} className={`w-full border-2 border-dashed rounded-xl p-4 flex flex-col items-center justify-center cursor-pointer transition-all focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 dark:focus:ring-offset-[#1a1f2e] ${isDraggingContext ? (isDarkMode ? "border-indigo-500 bg-indigo-900/20" : "border-indigo-500 bg-indigo-50") : isDarkMode ? "border-[#232a3b] bg-[#131722] hover:bg-[#0b0f19]" : "border-slate-300 bg-slate-50 hover:bg-slate-100"}`} onDragOver={(e) => { e.preventDefault(); setIsDraggingContext(true); }} onDragLeave={(e) => { e.preventDefault(); setIsDraggingContext(false); }} onDrop={handleContextDrop}>
                              <input type="file" multiple ref={contextInputRef} onChange={(e) => { if(e.target.files.length) { const file = e.target.files[0]; setContextFiles(prev => [...prev, ...Array.from(e.target.files)]); setContextTheme(generateThemeFromFile(file)); showToast("Context Attached! Theme adjusted."); } }} className="hidden" />
                    ...
                    {status === "uploading" && (
                      <motion.div key="uploading" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center justify-center py-10 gap-6 w-full max-w-lg mx-auto">
                        ...
                        <div className="text-center w-full">
                          ...
                          <p className={`font-medium text-sm mb-4 ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}>{processingType === "youtube" ? "Fetching Transcript from YouTube..." : "Uploading to Spoly Servers..."}</p>
                ...
                {(status === "recording" || status === "processing" || status === "paused") && (
                  <motion.div key="live-panel" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="grid xl:grid-cols-12 gap-8 h-[75vh]">
                    ...
                    <div className={`xl:col-span-4 flex flex-col h-full shadow-sm rounded-[2rem] border overflow-hidden transition-colors ${isDarkMode ? "bg-[#1a1f2e] border-[#2A2F3D]" : "bg-white border-slate-200"}`}>
                      ...
                      <div className={`relative p-5 flex items-center justify-between gap-4 border-b ${isDarkMode ? "bg-gradient-to-r from-[#0b0f19] to-[#131722] border-[#2A2F3D]" : "bg-gradient-to-r from-slate-50 to-white border-slate-200"}`}>
                         ...
                         <div className="flex items-center gap-3 shrink-0 z-10">
                           ...
                           <h2 className={`font-mono text-2xl sm:text-3xl font-medium tracking-tight ${status === "paused" ? "text-amber-500" : isDarkMode ? "text-white" : "text-slate-900"}`}>
                             {formatTime(timer)}
                           </h2>
                         </div>
                         ...
                         {!isExtensionActive && (status === "recording" || status === "paused") && (
                           <div className="flex items-center gap-2 z-10 shrink-0">
                             ...
                             >
                               {status === "paused" ? <Play size={16} fill="currentColor" className="ml-0.5" /> : <Pause size={16} fill="currentColor" />}
                             </motion.button>
                      ...
                      <div className={`p-4 px-6 border-b flex items-center gap-2 font-bold text-xs uppercase tracking-widest ${isDarkMode ? "bg-[#131722]/80 border-[#2A2F3D] text-slate-400" : "bg-slate-50 border-slate-100 text-slate-500"}`}>
                      ...
                      <div className="relative flex-1 min-h-0 bg-transparent">
                        <div 
                          ...
                          style={{ maskImage: 'linear-gradient(to bottom, rgba(0,0,0,0) 0%, rgba(0,0,0,1) 15%)', WebkitMaskImage: 'linear-gradient(to bottom, rgba(0,0,0,0) 0%, rgba(0,0,0,1) 15%)' }}
                        >
                          <p className={`font-sans font-medium leading-relaxed text-sm pr-2 pt-6 pb-12 ${isDarkMode ? "text-slate-200" : "text-slate-800"}`}>
                            {transcript}
                    ...
                    <div className={`xl:col-span-8 shadow-sm rounded-[2rem] p-8 flex flex-col relative transition-colors border ${isDarkMode ? "bg-[#1a1f2e] border-[#2A2F3D]" : "bg-white border-slate-200"}`}>
                      <div className={`flex items-center gap-3 mb-6 pb-4 border-b ${isDarkMode ? "border-[#2A2F3D]" : "border-slate-100"}`}><Zap size={24} className={status === "recording" ? "animate-pulse text-blue-500" : status === "paused" ? "text-amber-500" : "text-emerald-500"} /><h3 className="font-bold text-xl tracking-tight">{status === "recording" ? "AI generating notes..." : "Finalizing Document..."}</h3></div>
                      
                      <div className="flex-1 overflow-y-auto space-y-6 pr-2 custom-scrollbar">
                        <div className="space-y-2">
                          <div className={`flex items-center gap-2 font-bold text-sm ${isDarkMode ? "text-slate-300" : "text-slate-700"}`}>
                            <AlignLeft size={16} /> Summary
                          </div>
                          <div className={`p-5 rounded-2xl text-sm leading-relaxed border shadow-inner min-h-[100px] ${isDarkMode ? "text-slate-300 bg-[#0b0f19] border-[#2A2F3D]" : "text-slate-600 bg-slate-50 border-slate-100"}`}>
                            {meetingNotes.summary ? meetingNotes.summary : (
                               <div className="space-y-3 pt-1 w-full opacity-70">
                                 <div className={`h-2.5 w-full rounded-full animate-pulse ${isDarkMode ? 'bg-slate-800' : 'bg-slate-200'}`}></div>
                                 <div className={`h-2.5 w-5/6 rounded-full animate-pulse ${isDarkMode ? 'bg-slate-800' : 'bg-slate-200'}`}></div>
                                 <div className={`h-2.5 w-4/6 rounded-full animate-pulse ${isDarkMode ? 'bg-slate-800' : 'bg-slate-200'}`}></div>
                               </div>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="space-y-2">
                          <div className={`flex items-center gap-2 font-bold text-sm ${isDarkMode ? "text-emerald-400" : "text-emerald-600"}`}>
                            <ListChecks size={16} /> Action Items
                          </div>
                          <ul className="space-y-2">
                            {actionItems.length > 0 ? (
                              actionItems.map((item) => (
                                <li key={item.id} onClick={() => toggleActionItem(item.id)} className={`flex items-center gap-3 p-4 rounded-xl border cursor-pointer focus:outline-none focus:ring-2 focus:ring-emerald-500 ${item.done ? (isDarkMode ? "bg-[#0b0f19] border-[#2A2F3D] opacity-50" : "bg-slate-50 border-slate-200 opacity-60") : isDarkMode ? "bg-[#131722] border-[#2A2F3D]" : "bg-white border-slate-200"}`}>
                              ...
                              [1, 2].map(i => (
                                 <li key={i} className={`flex items-center gap-3 p-4 rounded-xl border opacity-70 ${isDarkMode ? 'border-[#2A2F3D] bg-[#131722]/50' : 'border-slate-200 bg-white/50'}`}>
                                    <div className={`w-4 h-4 rounded-full animate-pulse shrink-0 ${isDarkMode ? 'bg-slate-800' : 'bg-slate-200'}`}></div>
                                    <div className={`h-2.5 rounded-full animate-pulse w-3/4 ${isDarkMode ? 'bg-slate-800' : 'bg-slate-200'}`}></div>
                                 </li>
                              ))
                            )}
                          </ul>
                        ...
                        <div className="space-y-2 pt-2">
                          <div className={`flex items-center gap-2 font-bold text-sm ${isDarkMode ? "text-blue-400" : "text-blue-600"}`}>
                            <Workflow size={16} /> Live Diagram Generation
                          </div>
                          <div className={`p-6 rounded-2xl border shadow-inner flex items-center justify-center min-h-[250px] overflow-hidden ${isDarkMode ? "bg-[#0b0f19] border-[#2A2F3D]" : "bg-slate-50 border-slate-100"}`}>
                            {editableMermaid ? (
                              ...
                              <div className="flex flex-col items-center justify-center gap-4 w-full opacity-60">
                                 <div className={`w-16 h-12 rounded-xl animate-pulse ${isDarkMode ? 'bg-slate-800' : 'bg-slate-200'}`}></div>
                                 <div className={`w-0.5 h-8 animate-pulse ${isDarkMode ? 'bg-slate-800' : 'bg-slate-200'}`}></div>
                ...
                {status === "complete" && (
                  <motion.div key="success-panel" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="max-w-5xl mx-auto space-y-8">
                    ...
                    <AnimatePresence mode="wait">
                      {isRemixing ? (
                        <motion.div key="remixing-loader" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className={`rounded-[2.5rem] p-16 flex flex-col items-center justify-center text-center relative overflow-hidden shadow-2xl border ${isDarkMode ? "bg-[#131722] border-indigo-500/30" : "bg-white border-indigo-200"}`}>
                          <div className={`absolute top-0 left-1/2 -translate-x-1/2 w-64 h-64 blur-[80px] rounded-full pointer-events-none ${isDarkMode ? "bg-indigo-600/20" : "bg-indigo-400/20"}`}></div>
                          <RefreshCw size={48} className={`animate-spin mb-6 relative z-10 ${isDarkMode ? "text-indigo-400" : "text-indigo-600"}`} />
                          <h2 className={`text-3xl font-extrabold tracking-tight mb-3 relative z-10 ${isDarkMode ? "text-white" : "text-slate-900"}`}>Remixing your notes...</h2>
                          <p className={`font-medium relative z-10 ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}>Applying the <strong className={isDarkMode ? "text-indigo-300" : "text-indigo-700"}>{activeAiTemplate?.name}</strong> framework to the transcript.</p>
                        </motion.div>
                      ) : (
                        <motion.div key="success-dashboard" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="space-y-8">
                          ...
                          <div className={`rounded-[2.5rem] relative border ${isDarkMode ? "bg-[#0b0f19] border-emerald-500/20 shadow-[0_20px_60px_rgba(16,185,129,0.1)]" : "bg-gradient-to-br from-emerald-500 to-teal-500 text-white shadow-2xl border-transparent"}`}>
                            ...
                            <div className="relative z-20 p-8 md:p-12 flex flex-col md:flex-row justify-between items-center gap-6">
                              ...
                              <div className="flex items-center gap-6">
                                ...
                                <div>
                                  <h2 className={`text-3xl md:text-4xl font-extrabold tracking-tight mb-2 ${isDarkMode ? "text-emerald-50" : "text-white"}`}>Session Saved!</h2>
                                  <p className={`font-medium ${isDarkMode ? "text-emerald-200/70" : "text-emerald-50"}`}>
                                    Generated using <strong className="text-white">{activeAiTemplate ? activeAiTemplate.name : "Smart Document"}</strong> in {outputLanguage}.
                                  </p>
                                </div>
                              </div>
                              ...
                              <div className="flex flex-col sm:flex-row items-center gap-3">
                                <div className="relative">
                                  <button onClick={() => setShowRemixMenu(!showRemixMenu)} className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold border transition-all hover:scale-105 active:scale-95 focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:ring-offset-2 ${isDarkMode ? "bg-emerald-900/40 border-emerald-700/50 text-emerald-200 hover:bg-emerald-800/60 focus:ring-offset-[#0b0f19]" : "bg-emerald-700 border-emerald-600 text-white hover:bg-emerald-600 focus:ring-offset-emerald-500"}`}>
                                    <RefreshCw size={18} /> Remix Format
                                  </button>
                                  <AnimatePresence>
                                    {showRemixMenu && (
                                      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} className={`absolute top-full right-0 mt-3 w-72 border rounded-2xl shadow-2xl z-50 ${isDarkMode ? "bg-slate-800 border-slate-700" : "bg-white border-slate-200"}`}>
                                        <div className={`p-3 border-b text-xs font-bold uppercase tracking-widest ${isDarkMode ? "border-slate-700 text-slate-400" : "border-slate-100 text-slate-500"}`}>
                                          Select Template to Remix
                                        </div>
                                        <div className="max-h-[300px] overflow-y-auto custom-scrollbar p-2">
                                          {templatesDB.map((t) => (
                                            <button key={t.id} onClick={() => triggerRemix(t)} className={`w-full text-left px-3 py-3 rounded-xl mb-1 last:mb-0 flex items-center gap-3 text-sm font-bold transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 ${isDarkMode ? "hover:bg-slate-700 text-slate-200" : "hover:bg-slate-50 text-slate-700"}`}><span className={getTheme(t.theme, isDarkMode).text}>{t.icon}</span> {t.name}</button>
                                          ))}
                                        </div>
                                      </motion.div>
                                    )}
                                ...
                                <button onClick={() => { setSelectedNote(savedNotes[0]); setStatus("idle"); }} className={`flex items-center justify-center gap-2 px-8 py-3 rounded-xl font-bold transition-all hover:scale-105 active:scale-95 shadow-lg focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 ${isDarkMode ? "bg-emerald-500 text-slate-900 hover:bg-emerald-400 focus:ring-offset-[#0b0f19]" : "bg-white text-emerald-600 hover:bg-slate-50 focus:ring-offset-emerald-500"}`}>
                          ...
                          <div className="grid md:grid-cols-2 gap-6">
                            <div className={`rounded-[2rem] p-8 border shadow-sm ${isDarkMode ? "bg-[#1a1f2e] border-[#2A2F3D]" : "bg-white border-slate-200"}`}>
                               <div className={`flex items-center gap-2 font-bold mb-4 tracking-tight ${isDarkMode ? "text-slate-300" : "text-slate-700"}`}>
                                 <AlignLeft size={18} className={isDarkMode ? "text-blue-400" : "text-blue-600"} /> Extracted Summary
                               </div>
                               <p className={`text-sm leading-relaxed line-clamp-4 ${isDarkMode ? "text-slate-400" : "text-slate-600"}`}>
                            ...
                            <div className={`rounded-[2rem] p-8 border shadow-sm ${isDarkMode ? "bg-[#1a1f2e] border-[#2A2F3D]" : "bg-white border-slate-200"}`}>
                               <div className={`flex items-center gap-2 font-bold mb-4 tracking-tight ${isDarkMode ? "text-slate-300" : "text-slate-700"}`}>
                                 <ListChecks size={18} className={isDarkMode ? "text-emerald-400" : "text-emerald-600"} /> Pending Tasks
                               </div>
                               {actionItems.length > 0 ? (
                                 <ul className="space-y-3">
                                   {actionItems.slice(0, 3).map(item => (
                                     <li key={item.id} className={`flex items-start gap-3 text-sm font-medium ${isDarkMode ? "text-slate-400" : "text-slate-600"}`}>
                                 ...
                                 <p className={`text-sm italic ${isDarkMode ? "text-slate-500" : "text-slate-400"}`}>No action items detected.</p>
                               )}
                            </div>
                          </div>
                          ...
                          <div className="flex justify-center mt-12 pb-6">
                             <button onClick={handleReset} className={`group relative flex items-center justify-center gap-3 px-8 py-4 rounded-2xl font-bold text-lg transition-all duration-300 hover:scale-105 active:scale-95 overflow-hidden focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${isDarkMode ? "bg-slate-800 text-white shadow-[0_0_30px_rgba(37,99,235,0.2)] hover:shadow-[0_0_50px_rgba(37,99,235,0.4)] dark:focus:ring-offset-[#0b0f19]" : "bg-white text-slate-800 shadow-xl border border-slate-200 hover:border-blue-300 hover:shadow-[0_20px_40px_rgba(37,99,235,0.15)]"}`}>
                               ...
                               <span className={`relative z-10 flex items-center gap-2 transition-colors ${isDarkMode ? "group-hover:text-white" : "group-hover:text-blue-700"}`}>
                                 <PlusCircle size={22} className={`transition-colors ${isDarkMode ? "text-blue-400 group-hover:text-white" : "text-blue-500 group-hover:text-blue-600"}`} /> Start New Recording
                               </span>
                             </button>
                          </div>
