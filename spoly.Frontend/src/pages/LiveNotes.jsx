...
// 🚀 BULLETPROOF BACKEND INTEGRATION
  const processAudioWithBackend = async (audioBlob, defaultTitle) => {
    try {
      ...
      // 🚀 CRITICAL FIX: Upgraded Mermaid Super Sanitizer
      let generatedGraph = data.diagram || "";
      if (generatedGraph && generatedGraph !== "API FAILED") {
          // 1. Extract code from markdown backticks if they exist
          const match = generatedGraph.match(/\x60\x60\x60(?:mermaid)?\s*([\s\S]*?)\x60\x60\x60/i);
          if (match) {
              generatedGraph = match[1];
          }

          // 2. Slice off conversational text before the graph starts
          ...
          if (startIndex !== -1) {
              generatedGraph = generatedGraph.substring(startIndex);
          }

          // 3. AGGRESSIVE AUTO-CORRECTOR for AI Hallucinations
          generatedGraph = generatedGraph
              .replace(/\x60\x60\x60/g, "") // Catch stray backticks
              .replace(/\]\s*graph\s+subgraph/gi, "]\nsubgraph") 
              .replace(/graph\s+subgraph/gi, "subgraph") 
              .replace(/\]\s*subgraph/gi, "]\nsubgraph") 
              .replace(/end\s+subgraph/gi, "end\nsubgraph") 
              .replace(/\]\s+([A-Za-z0-9_]+)\[/g, "]\n$1[") 
              .replace(/^[ \t]*note\s+.*$/gim, "") // 🚀 THE NEW FIX: Nukes any invalid "note right of" lines completely!
              .trim();
      } else {
          generatedGraph = "";
      }
