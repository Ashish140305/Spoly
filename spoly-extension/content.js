console.log("✅ Spoly Extension Connected! (Sleek Panel & Fixed Drag)");
...
function injectNexusBot() {
  ...
  // 1. Host Container (Using standard bottom/right to start, no conflicting transforms)
  const host = document.createElement('div');
  host.id = 'spoly-fab-root';
  host.style.cssText = 'position: fixed; bottom: 30px; right: 30px; z-index: 2147483647; display: flex; flex-direction: column; align-items: flex-end; gap: 16px; filter: drop-shadow(0 10px 20px rgba(0,0,0,0.3));';
  ...
  style.textContent = `
    * { box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, "Inter", "Segoe UI", Roboto, sans-serif; margin: 0; padding: 0; }
    ...
    .panel {
      ...
      opacity: 0; transform: translateY(10px) scale(0.98); transform-origin: bottom right;
    ...
    .title-badge { 
      ...
      padding: 4px 10px; border-radius: 12px; font-size: 11px; font-weight: 700; color: #60a5fa; 
      display: flex; align-items: center; gap: 6px; text-transform: uppercase; letter-spacing: 0.5px;
    }
    .close-btn { background: transparent; border: none; color: #64748b; cursor: pointer; width: 24px; height: 24px; display: flex; align-items: center; justify-content: center; transition: 0.2s; border-radius: 50%; }
    .close-btn:hover { background: rgba(255,255,255,0.1); color: white; }

    /* Compact Visualizer */
    .visualizer { background: rgba(0,0,0,0.4); border-radius: 12px; padding: 12px 16px; display: flex; align-items: center; justify-content: space-between; margin-bottom: 16px; border: 1px solid rgba(255,255,255,0.03); }
    ...
    .timer { font-family: ui-monospace, monospace; font-size: 12px; font-weight: 600; color: #64748b; letter-spacing: 0.5px; }
    .panel.recording .timer { color: #fca5a5; }

    /* Minimalist Buttons */
    .btn { width: 100%; padding: 12px; border-radius: 12px; font-weight: 600; font-size: 13px; cursor: pointer; border: none; transition: all 0.2s; display: flex; align-items: center; justify-content: center; gap: 8px; color: white; letter-spacing: 0.3px; }
    .btn-start { background: linear-gradient(135deg, #2563eb, #4f46e5); box-shadow: 0 4px 12px rgba(37,99,235,0.3), inset 0 1px 1px rgba(255,255,255,0.2); }
    .btn-start:hover { filter: brightness(1.1); transform: translateY(-1px); box-shadow: 0 6px 16px rgba(37,99,235,0.4), inset 0 1px 1px rgba(255,255,255,0.2); }
    .btn-stop { background: linear-gradient(135deg, #dc2626, #9f1239); display: none; box-shadow: 0 4px 12px rgba(220,38,38,0.3), inset 0 1px 1px rgba(255,255,255,0.2); }
    .btn-stop:hover { filter: brightness(1.1); transform: translateY(-1px); box-shadow: 0 6px 16px rgba(220,38,38,0.4), inset 0 1px 1px rgba(255,255,255,0.2); }

    /* --- THE NEXUS AI CORE --- */
    .nexus-container { width: 60px; height: 60px; position: relative; cursor: grab; display: flex; align-items: center; justify-content: center; }
    ...
    .nexus-core { position: relative; width: 40px; height: 40px; border-radius: 50%; background: rgba(15, 23, 42, 0.9); backdrop-filter: blur(8px); border: 1px solid rgba(255,255,255,0.1); box-shadow: inset 0 0 12px rgba(59,130,246,0.3); display: flex; align-items: center; justify-content: center; transition: all 0.3s ease; }
    ...
    @keyframes breathe { 0%, 100% { transform: rotate(45deg) scale(1); } 50% { transform: rotate(45deg) scale(1.2); } }
    @keyframes heartBeat { 0% { transform: scale(0.8); } 100% { transform: scale(1.3); } }
    @keyframes bounceWave { 0%, 100% { height: 20%; } 50% { height: 90%; } }
  `;
    ...
    <div class="panel" id="spoly-panel">
      <div class="header">
        <div class="title-badge">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><path d="m12 19-7-7 7-7"/><path d="M19 12H5"/></svg>
          Spoly
        </div>
        <button class="close-btn" id="close-panel"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" x2="6" y1="6" y2="18"/><line x1="6" x2="18" y1="6" y2="18"/></svg></button>
      </div>
      ...
      <button class="btn btn-stop" id="stopBtn">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><rect width="16" height="16" x="4" y="4" rx="3"/></svg>
        Stop & Save
      </button>
    </div>

    <div class="nexus-container" id="nexus-trigger">
      <div class="orbital-ring"></div>
      <div class="orbital-ring-inner"></div>
      <div class="nexus-core"><div class="spark"></div></div>
    </div>
  `;
  ...
  // ==========================================
  // 🛡️ BULLETPROOF DRAG MECHANICS
  // ==========================================
  let isDragging = false, hasMoved = false;
  let startX, startY, initialLeft, initialTop;

  nexusTrigger.addEventListener('mousedown', (e) => {
    isDragging = true;
    hasMoved = false;
    startX = e.clientX;
    startY = e.clientY;

    // Grab the exact current position of the container
    const rect = host.getBoundingClientRect();
    initialLeft = rect.left;
    initialTop = rect.top;

    // Switch positioning from bottom/right to exact left/top coordinates
  ...
  window.addEventListener('mousemove', (e) => {
    ...
    // If mouse moves more than 3px, it counts as a drag, not a click
    ...
    // Clamp to screen boundaries so it can't be dragged off-screen
  ...
  startBtn.onclick = async () => {
    try {
      ...
      seconds = 0; timerDisplay.textContent = "00:00"; 
...
chrome.runtime.onMessage.addListener((req) => { 
  if (req.action === 'toggle_widget') injectNexusBot(); 
  if (req.action === 'RECORDING_STARTED') window.postMessage({ type: 'SPOLY_RECORDING_STARTED' }, '*'); 
  if (req.action === 'RECORDING_STOPPED') window.postMessage({ type: 'SPOLY_RECORDING_STOPPED' }, '*'); 
});

window.addEventListener('message', (e) => { 
  if (e.data.type === 'SPOLY_LAUNCH_WIDGET') injectNexusBot(); 
});
